---
title: "Data Cleaning for Severe Injuries"
...
The file deals with processing the OSHA Injury Database by cleaning and transforming various columns, including `EventDate`, `Primary NAICS`, `Nature`, `Part of Body`, and `Event`. For each of these columns, you are creating new categorical variables based on patterns found in the data, such as classifying the NAICS code into industries, the nature of the injury, the body part affected, and the type of event. You are also converting the EventDate into a year column and filtering out rows with missing or invalid NAICS codes.
...
The below piece of code is used for dropping NA and irrelevant values from `Primary NAICS` column.

```{r}
injury_data <- injury_data[!is.na(injury_data$`Primary NAICS`) & injury_data$`Primary NAICS` != 999999, ]
```

The following code is used to extract just the year from the column `EventData` as we will be just dealing with the year in our Shiny app.
...
The code snippet categorizes industries based on `Primary NAICS` codes into broader sectors using `case_when()` and regex pattern matching. It maps each NAICS code to a specific level industry category and assigns "Uncategorized" for codes that don't match predefined patterns.
...
The code snippet categorizes injury types based on the `Nature` column by matching specific numeric codes to predefined categories using `case_when()` and regex. It helps organize diverse injury types into meaningful groups for easier analysis and reporting.
...
The code snippet assigns categories to the `Part of Body` column by mapping specific numeric codes to descriptive body part groups using `case_when()` and regex. It streamlines the data for analysis by grouping similar body parts into broader, meaningful categories.
...
The code snippet assigns descriptive categories to the `Event` column by matching specific numeric codes to predefined event groups using `case_when()` and regex. It helps classify events into meaningful categories for further analysis or reporting.
...
The code snippet adds a new column `source_category` to the injury_data dataframe based on specific patterns found in the Source column. Using the `case_when()` function, it categorizes Source values into different groups like "Chemicals", "Tools and Equipments", "Machinery", etc., depending on matching regex patterns.
...
The code cleans and selects relevant columns from the injury_data dataset, making it more manageable for analysis. By renaming the columns and filtering out rows with an "Uncategorized" industry, it improves both the clarity and the quality of the data for further processing.
...
The code saves the processed injury_data dataframe to a CSV file at the specified file_path.
